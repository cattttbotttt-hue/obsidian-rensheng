...
5. **每次改完筆記前後，喺 Termux 手動同步**（習慣養成）  
   ```bash
   cd /sdcard/Download/obsidian-rensheng
   git pull
   git add .
   ...
   **⚠️ 若出現 `fatal: detected dubious ownership`（唔信任倉庫擁有者）**  
   `/sdcard` 上嘅資料夾同 Termux 用戶唔同，Git 2.36+ 會**拒絕**任何指令，所以你会觉得「電腦同手機唔 sync」——手機根本上唔到 GitHub。  
   **只做一次**：喺 Termux 執行（路徑要同你 vault 一致；下面係 `Download` 版本）：
   ```bash
   git config --global --add safe.directory /storage/emulated/0/Download/obsidian-rensheng
   ```
   再重新跑同步腳本或 `git pull` / `git push`。
